...
async def quality_callback(client: Client, query: CallbackQuery):
    ...
    if not url:
        try:
            await query.message.edit_text("⚠️ Session expired. Please send the URL again.")
        except Exception:
            pass
        return
...
async def audio_cmd(client: Client, message: Message):
    ...
    status_msg = await message.reply_text("🎵 **Queuing audio extraction...**")
...
async def handle_document(client: Client, message: Message):
    ...
    try:
        ...
        await status_msg.edit_text(
            ...
            f"{BULLET} Queueing all..."
        )

        for i, url in enumerate(lines, 1):
            s_msg = await message.reply_text(f"⏳ `[{i}/{len(lines)}]` Queuing `{url[:40]}...`")
